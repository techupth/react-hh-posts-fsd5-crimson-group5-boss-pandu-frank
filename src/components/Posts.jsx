...
function Posts() {
  const [addPost, setAddPost] = useState(information);

  const handleLike = (postIndex) => {
    let newPostlist = [...addPost];
    newPostlist[postIndex].likes += 1;
    setAddPost(newPostlist);
  };
  ...
  return (
    <div class="app-wrapper">
      ...
      <div class="post-list">
        {addPost.map((information, postIndex) => {
          return (
            <div class="post-item" key={information.id}>
              <div class="post-header">
                <h2>{information.title}</h2>
                <div class="post-social-media-stats">
                  <span class="stats-topic">Like: </span>
                  <span class="post-likes">{information.likes}</span>
                </div>
              </div>
              <p class="post-content">{information.content}</p>
